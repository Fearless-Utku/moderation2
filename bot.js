...
client.on("message", message => {
    if(message.content.toLowerCase() == "r!tag") 
    return message.channel.send(`⚝`)
});
...
client.on("message", message => {
    ...
    return message.channel.send(`https://discord.gg/Mne72evGsM Sunucumuzun Sınırsız Davet Linki`)
});
...
let rol = member.guild.roles.cache.find(r => r.name === "⛓ Punitive");
...
db.delete(`cezali_${member.guild.id + member.id}`)
    ...
    member.roles.remove('780479085422968894');
  }, ms(sürejail));
}
})
...
db.delete(`muteli_${member.guild.id + member.id}`)
    ...
    member.roles.remove('780479515607564288');
  }, ms(süre));
}
})
 ...
 const i = await db.fetch(`kufur_${msg.guild.id}`)
    if (i == "acik") {
        ...
        if (kufur.some(word => msg.content.includes(word))) {
          try {
            if (!msg.member.hasPermission("780002075965653022")) {
                  msg.delete();
 ...
 const i = db.fetch(`${oldMessage.guild.id}.kufur`)
    if (i) {
        ...
        if (kufur.some(word => newMessage.content.includes(word))) {
          try {
            if (!oldMessage.member.hasPermission("780002075965653022")) {
                  oldMessage.delete();
